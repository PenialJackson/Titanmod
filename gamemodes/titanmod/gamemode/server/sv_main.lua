...
local gravityCVar = GetConVar("sv_tm_player_gravity")
...
local crouchEnterCVar = GetConVar("sv_tm_player_crouch_time_enter")
local crouchExitCVar = GetConVar("sv_tm_player_crouch_time_exit")
...
function GM:PlayerSpawn(ply)
	...
	ply:SetJumpPower(150 * jumpMultCVar:GetInt())
	...
	if GetGlobal2Bool("tm_intermission") then
		ply:Freeze(true)
	end
...
if table.HasValue(AVAILABLEMAPS, game.GetMap()) then
	...
	function EndMatch()
		...
		for _, v in RandomPairs(GAMEMODES) do
			if activeGamemode != v and v[4] == true then
				table.insert(modePool, v[1])
			end
		end

		for _, v in RandomPairs(GAMEMODES) do
			if activeGamemode != v and v[4] == false then
				table.insert(modePoolSecondary, v[1])
			end
		end
	...
	local function MatchStatusCheck()
		local ct = CurTime()
		local currentTime = math.Round(GetGlobal2Int("tm_matchtime", 0) - ct)

		if ct > GetGlobal2Int("tm_matchtime", 0) then
			EndMatch()
		end
		...
		if revote == false then
			for k, v in ipairs(AVAILABLEMAPS) do
				if v == votedMap then
					...
					if mapIndex	== 1 then
						SetGlobal2Int("VotesOnMapOne", GetGlobal2Int("VotesOnMapOne", 0) + 1)
					elseif mapIndex == 2 then
						SetGlobal2Int("VotesOnMapTwo", GetGlobal2Int("VotesOnMapTwo", 0) + 1)
					elseif mapIndex == 3 then
						SetGlobal2Int("VotesOnMapThree", GetGlobal2Int("VotesOnMapThree", 0) + 1)
					end
				end
			end
		else
			...
			for k, v in ipairs(AVAILABLEMAPS) do
				if v == votedMap then
					...
					if mapIndex	== 1 then
						SetGlobal2Int("VotesOnMapOne", GetGlobal2Int("VotesOnMapOne", 0) + 1)
						SetGlobal2Int(unvotedMapInt, GetGlobal2Int(unvotedMapInt, 0) - 1)
					elseif mapIndex == 2 then
						SetGlobal2Int("VotesOnMapTwo", GetGlobal2Int("VotesOnMapTwo", 0) + 1)
						SetGlobal2Int(unvotedMapInt, GetGlobal2Int(unvotedMapInt, 0) - 1)
					elseif mapIndex == 3 then
		...
		if revote == false then
			for k, v in ipairs(GAMEMODES) do
				if v[1] == votedMode then
					...
					if modeIndex == 1 then
						SetGlobal2Int("VotesOnModeOne", GetGlobal2Int("VotesOnModeOne", 0) + 1)
					elseif modeIndex == 2 then
						SetGlobal2Int("VotesOnModeTwo", GetGlobal2Int("VotesOnModeTwo", 0) + 1)
					end
				end
			end
		else
...
function AddMatchTimeCommand(ply, cmd, args)
	if ply:IsAdmin() then
		SetGlobal2Int("tm_matchtime", GetGlobal2Int("tm_matchtime", matchLength:GetInt()) + args[1])
	end
end
...
hook.Add("PlayerCanHearPlayersVoice", "ProxVOIP", function(listener,talker)
	if (tonumber(listener:GetPos():Distance(talker:GetPos())) > voiceRange:GetInt()) then
		return false, false
	else
		return true, true
	end
