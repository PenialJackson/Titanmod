...
function SWEP:SmackDamage(tr, fwd, primary)
	...
	if primary then
		...
		if tr.Entity:IsPlayer() then
			local attack_angle = self:GetOwner():GetAimVector():Angle().yaw - tr.Entity:GetAimVector():Angle().yaw

			if (math.abs(attack_angle) < 90) or (math.abs(attack_angle) > 270) then
				dmg = 500
				self:EmitSound(Sound("Weapon_BO3_AXE.Hit"))
			end
		end
	end
...
function SWEP:ThrowKnife()
	...
	if SERVER then
		local ent = ents.Create("tm_thrown_blade")
